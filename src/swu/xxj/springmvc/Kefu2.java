 ...
 */
@WebServlet("/Kefu2")
public class Kefu2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("user_id",user_id);
		 ...
		 java.text.SimpleDateFormat formatter=new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			...
			String login_time=formatter.format(currentTime);
		
		List<Kefu_jilu> kefu_jilu_list=new ArrayList<Kefu_jilu>();
		kefu_jilu_list=Test.getKefu_jilu();
		
		request.setAttribute("kefu_jilu_list",kefu_jilu_list);
		request.setAttribute("login_time",login_time);
		request.getRequestDispatcher("admin/kefu.jsp").forward(request, response);
	}
